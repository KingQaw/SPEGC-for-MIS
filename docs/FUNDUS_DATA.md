...
### 结果 I：按论文修第一处（σ 可学习）—— **单独修不够，且会把模型打崩**

按论文附录 A 的 Algorithm 1 第 32 行（σ 在可学习集合里）去掉 `detach`。
改动做成**可选开关** `SEMISUPNET.SPEGC_TTT_GRAD`（默认 `False` = 发布版行为），
以免默认路径被破坏。

**开关效果（`model_D` 在 RIM-ONE 上，实测变化张量数）：**

| `SPEGC_TTT_GRAD` | 检测网络变化 | backbone 梯度非零元素 |
|---|---|---|
| `False`（默认，发布版） | **0 / 95** | 0 |
| `True`（按论文） | **58 / 95** | > 0 |
...
| 适应学习率 | 有效适应步 | 视杯 Dice | 视盘 Dice | 两类均值 |
|---|---|---|---|---|
| 0.005（config 默认，论文值） | **1** | 0.00 | 0.00 | **0.00** |
| 1e-4 | 38 | 0.00 | 57.23 | ~28.6 |
| 1e-5 | 39 | 0.00 | 76.25 | ~38.1 |
| **1e-6** | 39 | 29.24 | 90.56 | ~59.9 |
| 无 TTT（基线） | — | — | — | 57.75 |

机制：第一步梯度把检测器推爆 → `graph_generator` 产不出节点 → TTT 分支提前
`return None` → `update_pool` 不被调用 → 图池永远填不满 → 后续步全部跳过。
所以 **`LR=0.005` 时只有 1 步真正发生，而那一步就足以让预测数归零**。

即使把学习率降到 1e-6 勉强稳定，两类均值 ~59.9 对比基线 57.75，**增益在噪声
范围内**，而且视杯仍在 29 左右。

**结论：只修 `detach` 不能复现论文的方法。** 还需要第二处修正——论文
Algorithm 1 第 29 行说 `P` 是 **`f_σ` 给出的语义预测 `∈ℝ^{V×C}`**，而当前
`spegc.py` 算的是 `softmax(V* @ centroids.T)`（对 Z=48 个聚类中心的分配）。
`L_G = ΣΣ S*_ij·KL(P_j||sg(P_i))` 被套在聚类分配上时，对整网反传极不稳定；
换成论文定义的语义预测后才可能既稳定又有效。

**这一处我没有改**，因为改动涉及从 ROI 头取逐节点类别分布，需要重新设计
`graph_generator` 的输出接口，我没有足够依据确认论文的具体实现方式。

当前状态：`detach` 的修复以开关形式就位、默认关闭，仓库行为与发布版一致，
不会因为这次改动而破坏。
