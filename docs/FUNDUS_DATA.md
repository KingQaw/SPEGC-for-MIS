...
1. **RIM-ONE（−54.43）** —— 见上面「结果 E 的残余 1」，取景/版本不匹配。
   `datasets/raw/RIM-ONE_DL` 已下载，但**参考分割是独立下载**，尚未取得
   （见本文档「RIM-ONE DL」一节）。转换器已就绪：
   `.venv/bin/python tools/prepare_fundus_data.py --datasets RIM_ONE_r3 --rimone-source dl`
