...
### 结果 E：跨域迁移暴露的**取景不一致**问题（未解决）

用自训权重的按类口径跑留一法（`bash tools/run_loo_per_class.sh`，每集取 40 张
的快测），RIM-ONE 这一列塌了：

| 测试域 | 本项目 | 论文 |
|---|---|---|
| A (RIM-ONE) | **36.35** | 84.90 |
| B (REFUGE) | 87.65 | 83.34 |
| C (ORIGA) | 81.78 | 84.57 |
| D (REFUGE-Test) | 79.13 | 83.54 |
| E (Drishti-GS) | 78.26 | 85.51 |

注意 `model_A` 在 RIM-ONE 上**域内是 92.71**（结果 D 的表），但其他三个源模型
迁移过去只有 8.8~66.3。所以 RIM-ONE 本身可学，是**跨域迁移**失败。

**根因：四个域的取景尺度完全不同。** 实测裁剪后图像的非黑占比：
...
行/列非黑剖面能看出形态差异——REFUGE 呈典型的圆弦长分布
（`0.07 → 1.00 → 0.07`），而 RIM-ONE 是**平坦的 0.99**：
...
**为什么 padding 解决不了**：RIM-ONE 的圆形边界已经超出画面，无法从可见部分
反推圆的完整范围。

**最可能的原因**：论文 4.1 节写的是 `Domain A (RIM-ONE ¹⁷)`，引用的是**文献 17**
那一版 RIM-ONE。我们手上的是 **RIM-ONE r3**（来自 `medimrg.webs.ull.es`），
而学术界常用的还有 **RIM-ONE DL** 等版本，后者提供已裁好的单张眼底图。
**版本不同，取景自然不同**——这很可能是残差的真正来源。

**下一步**（择一）：

1. 换用 RIM-ONE DL（<https://github.com/miag-ull/rim-one-dl>，公开可直接下载），
   重新转换后重跑留一法；
2. 或者从可见圆弧拟合圆、外推出完整视网膜范围再裁剪；
3. 或者接受 RIM-ONE 不可比，在报告里明确标注该列作废。
